...
All four models were implemented using scikit-learn, wrapped in a single
...
($r \approx 0.57$), the strongest of any single numeric feature, consistent with it emerging
...
right-skew tail), consistent with a linear model struggling to capture the more extreme,
...
the right-skewed target distribution seen in Figure 1. This indicates the constant-variance
...
\textbf{Note on CV vs. test $R^2$:} All four models score noticeably higher $R^2$ on
...
$\rightarrow$ 0.34), consistent with a bias-dominated, not variance-dominated, problem.
...
0.16--0.16). This is a genuine but modest improvement -- regularization helped, but the
...
\end{itemize}
\textbf{Conclusion of this section:} The dominant issue for this dataset is
\textbf{underfitting (high bias)}, not overfitting. All four models plateau at a similar,
...
All four models converge to a similar, modest performance ceiling (test $R^2$ between 0.09
and 0.16), with \textbf{Ridge Regression} emerging as the best-performing model on both
...
weak signal rather than being pure noise. The overall pattern across every diagnostic
...
additional explanatory power. A meaningful next step would be engineering additional
...
The trade-off between accuracy and model complexity in this experiment is therefore small:
